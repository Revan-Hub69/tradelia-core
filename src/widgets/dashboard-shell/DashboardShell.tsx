/**
 * Dashboard Shell - Tradelia 2026
 * 
 * Layout principale della dashboard con sidebar intelligente,
 * header e area contenuto principale
 */

'use client';

import { type ReactNode } from 'react';
import { DashboardSidebar } from '@/widgets/sidebar/DashboardSidebar';
import { DashboardHeader } from '@/widgets/header/DashboardHeader';
import { CommandPalette } from '@/features/command-palette/components/CommandPalette';
import { createDefaultCommands } from '@/features/command-palette/lib/default-commands';
import { useSidebarState, useSidebarStore } from '@/features/sidebar-state';

interface DashboardShellProps {
  children: ReactNode;
}

export function DashboardShell({ children }: DashboardShellProps) {
  const sidebarState = useSidebarState();
  const { toggle: toggleSidebar } = useSidebarStore();
  
  // Create default commands for command palette
  const commands = createDefaultCommands();

  return (
    <div className="fixed inset-0 flex bg-background overflow-hidden">
      {/* Sidebar */}
      <DashboardSidebar 
        navigationItems={[
          {
            id: 'dashboard',
            label: 'Dashboard',
            href: '/dashboard',
            icon: ({ className }) => (
              <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 5a2 2 0 012-2h4a2 2 0 012 2v6H8V5z" />
              </svg>
            ),
            isActive: true
          },
          {
            id: 'analytics',
            label: 'Analytics',
            href: '/analytics',
            icon: ({ className }) => (
              <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
              </svg>
            ),
            isActive: false
          },
          {
            id: 'settings',
            label: 'Impostazioni',
            href: '/settings',
            icon: ({ className }) => (
              <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              </svg>
            ),
            isActive: false
          }
        ]}
      />
      
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <DashboardHeader 
          onToggleSidebar={toggleSidebar}
          sidebarState={sidebarState}
        />
        
        {/* Content - single scroll point */}
        <main className="flex-1 overflow-y-auto overscroll-none">
          <div className="p-4 sm:p-6 max-w-7xl mx-auto">
            {children}
          </div>
        </main>

        {/* Mini Footer */}
        <footer className="border-t border-border/50 px-4 py-2 bg-background">
          <div className="flex items-center justify-between text-[10px] text-muted-foreground max-w-7xl mx-auto">
            <span>© 2026 Tradelia. Strumento educativo.</span>
            <span>Non costituisce consulenza finanziaria.</span>
          </div>
        </footer>
      </div>

      {/* Command Palette */}
      <CommandPalette commands={commands} />
    </div>
  );
}
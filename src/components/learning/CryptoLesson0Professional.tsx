  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 px-4 py-8">
      ...
      <div className="text-center space-y-6">
        ...
        <div className="mx-auto max-w-md space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            <span className="font-medium">{exploredApproaches.size}</span> di <span className="font-medium">3</span> approcci esplorati
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="relative z-10">
                ...
                <h3 className={`font-semibold mb-2 ${isActive ? approach.color.text : 'text-foreground'}`}>
                  {approach.title}
                </h3>
              ...
              <div key={index} className="space-y-3">
                ...
                {section.type === 'highlight' && (
                  <div className={`rounded-lg border p-4 ${activeApproachData.color.border} ${activeApproachData.color.background}`}>
                    <h4 className={`font-semibold mb-3 ${activeApproachData.color.text}`}>
                      {section.content}
                    </h4>
                    {section.items && (
                ...
                {section.type === 'warning' && (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
                    <div className="flex items-start gap-3">
                      <svg className="size-5 text-amber-600 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
                      </svg>
                      <p className="text-sm text-amber-800">
                        <span className="font-medium">Nota importante:</span> {section.content}
                      </p>
                    </div>
                  </div>
                )}
      ...
      <div className="text-center">
        {isMasteryAchieved ? (
          <Button size="lg" className="h-12 px-8 text-base bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90">

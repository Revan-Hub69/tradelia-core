...
const BentoTile = ({ icon, titleKey, descKey, accent }: TileDef) => {
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-2xl border border-border/40 bg-card/50 p-6 transition-all duration-300 hover:border-border/70 hover:bg-card/80 hover:shadow-lg sm:rounded-[20px]">
      ...
      <div className={`relative mt-4 flex items-center gap-2 text-xs font-medium ${colors.text} opacity-0 transition-all duration-300 group-hover:opacity-100`}>
    ...
    <section className="border-t border-border/40 py-14 sm:py-16 lg:py-20 xl:py-24">
      <SectionContainer size="wide">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {tiles.map((tile, i) => (
            <FadeIn key={tile.titleKey} delay={i * 80}>
              <BentoTile {...tile} />
            </FadeIn>
          ))}
        </div>

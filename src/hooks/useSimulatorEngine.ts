...
export type SimulatorResult = {
  ...
  spreadCost: number;          // EUR assoluto
...
/**
 * Stub engine — sostituire con chiamata a normalizeToExposure + computeInstrumentScore
 * quando il domain module sarà wrappato in un'API route o importato direttamente.
 */
function runEngine(input: EngineInput): SimulatorResult[] {
  const { exposure, assetClass } = input;
  if (exposure < 100) return [];

  // Mock results — struttura identica all'output reale atteso
  const base = [
    {
      ...
      score: 84,
      feasibility: 'OPTIMAL' as Feasibility,
      spreadCost: exposure * 0.0008,
      commissionCost: 0,
      overnightCost: exposure * 0.0002,
      slippageCost: exposure * 0.0001,
      achievableExposure: exposure,
      deviationPct: 0.2,
    },
    {
      id: `${assetClass}-future-cme`,
      instrumentName: `${assetClass} Futures`,
      brokerName: 'Interactive Brokers',
      score: 71,
      feasibility: exposure >= 5000 ? 'FEASIBLE' as Feasibility : 'WARNING' as Feasibility,
      spreadCost: exposure * 0.0003,
      commissionCost: exposure * 0.0006,
      overnightCost: 0,
      slippageCost: exposure * 0.0002,
    ...
    },
    {
      ...
      brokerName: 'XTB',
      score: 58,
      feasibility: 'FEASIBLE' as Feasibility,
      spreadCost: exposure * 0.0015,
      commissionCost: exposure * 0.0004,
      overnightCost: 0,
      slippageCost: exposure * 0.0003,
      achievableExposure: exposure,
      deviationPct: 1.1,
    },
    {
      ...
      brokerName: 'Deribit',
      score: 41,
      feasibility: exposure >= 2000 ? 'WARNING' as Feasibility : 'INFEASIBLE' as Feasibility,
      spreadCost: exposure * 0.002,
      commissionCost: exposure * 0.0008,
      overnightCost: 0,
      slippageCost: exposure * 0.0005,

...
export const heroMonitorRows = [
  ...
  },
  {
    ...
    metaKey: 'monitor_row_2_meta',
    noteKey: 'monitor_row_2_note',
    dominantKey: 'monitor_dominant_holding',
    intensity: 57,
...
export const scenarioLeverageRange = {
  min: 1,
  max: 12,
} as const;

export function getLandingSectionHref(locale: string, pathname: string, sectionId: LandingSectionId): string {
  if (pathname === AppConfig.routes.home) {
    return `#${sectionId}`;
  }

  const localePrefix = locale !== AppConfig.defaultLocale
    ? `/${locale}`
    : '';
